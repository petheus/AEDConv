...
class Conv_block(nn.Module):
    def __init__(self, filtersin, filters, imsize, block_nums=2, kernel_size=[3, 3]):
        super(Conv_block, self).__init__()
        self.conv0 = nn.Conv2d(filtersin, filters, kernel_size=kernel_size, padding='same', dilation=(1, 1))
        ...
        self.conv1 = nn.Conv2d(filters, filters, kernel_size=kernel_size, padding='same', dilation=(1, 1))
        #self.conv1_1 = tf.keras.layers.Conv2D(filters=filters, kernel_size=kernel_size, padding='same', activation=None, dilation_rate=(1, 1))
        #self.convdep1 = tf.keras.layers.DepthwiseConv2D(kernel_size=kernel_size, padding='same')
        self.batch1 = nn.BatchNorm2d(filters, track_running_stats=False)
    ...
    def forward(self, inputs):
        ...
        x0 = self.conv1(x0)
        #x0 = self.conv1_1(x0, training=training)
        #x0 = tf.concat([x0_0,x0_1],axis=-1)
        #x0 = self.convdep1(x0)
        x0 = self.batch1(x0)
        x0 = torch.nn.functional.gelu(x0)
...
class Conv_block4(nn.Module):
    def __init__(self, filtersin, filters, imsize=None, block_nums=2, kernel_size=[3, 3]):
        ...
        self.conv0 = nn.Conv2d(filtersin, filters, kernel_size=[3, 3], padding='same', dilation=(1, 1))
        self.conv0_ = nn.Conv2d(filters, filters, kernel_size=[3, 3], padding='same', dilation=(2, 2))
        #self.batch0 = torch.nn.InstanceNorm2d(filters, track_running_stats=False)
        self.batch0_ = torch.nn.InstanceNorm2d(filters, track_running_stats=False)

        self.conv1 = nn.Conv2d(filters, filters, kernel_size=[3, 3], padding='same', dilation=(1, 1))
        self.conv1_ = nn.Conv2d(filters, filters, kernel_size=[3, 3], padding='same', dilation=(2, 2))
...
class Convde(nn.Module):
    def __init__(self, filtersin, filters, dilation=(1, 1), kernel_size=[3, 3], padding='same'):
        ...
        w11 = torch.empty(filters, filtersin, 3, 3)
        nn.init.kaiming_uniform_(w11, a=math.sqrt(5))
        self.weight11 = nn.Parameter(w11)
        ker11 = np.ones([filters, filtersin, 3, 3], dtype=np.float32)
        ker11[:, :,1,1] = ker0
        ker11[:, :,2,1] = ker0
        ker11[:, :,0,1] = ker0
        #ker11[:, :,1,0] *= 2
        #ker11[:, :,1,2] *= 2
        self.ker11 = torch.tensor(ker11, dtype=torch.float32, device=torch.device('cuda:0'))
        ...
        self.batchc00 = torch.nn.InstanceNorm2d(filters*2, track_running_stats=False, affine=False)
    ...
    def forward(self, inputs):
        ...
        w1 = self.weight1*self.ker1
        mean = torch.sum(w1, dim=(2,3), keepdim=True)/8
        x1 = (self.weight1-mean)*self.ker1
        #w11 = torch.where(self.ker11>0.5, self.weight11, 0)
        w11 = self.weight11*self.ker11
        mean = torch.sum(w11, dim=(2,3), keepdim=True)/8
        x11 = (self.weight11-mean)*self.ker1
        
        x00 = F.conv2d(inputs, x11, padding='same', dilation=self.dilation)
        #x0 = F.conv2d(inputs, x1 *  torch.where(self.ker2>0.5, F.sigmoid(x2)+1, 1) * torch.where(self.ker3>0.5, F.sigmoid(x3)+1, 1), padding='same', dilation=self.dilation)
        xo = F.conv2d(inputs, self.weighto, padding='same', dilation=self.dilation)
        #x0 = F.conv2d(inputs, torch.where(self.ker1>0.5, x1*2.25, x1), padding='same', dilation=self.dilation)
        x0 = F.conv2d(inputs, x1, padding='same', dilation=self.dilation)
        x0 = self.batchc0(x0)
        xo = self.batchco(xo)
        x00 = self.batchc00(x00)
